package dev.wyedusk.duskthings.effect;
...
public class SpectralTransformationMobEffect extends MobEffect {
    ...
    @Override
    public void onMobHurt(
            @NotNull LivingEntity entity, int amplifier, @NotNull DamageSource source, float amount) {
        if (amount > 0 && !entity.level().isClientSide()) {
            entity.removeEffect(DuskThings.SPECTRAL_TRANSFORMATION_MOB_EFFECT.getDelegate());
        }
    }
}
